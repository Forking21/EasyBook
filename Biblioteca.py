...
def InicioSesion():
    os.system("cls")
    print("Ingrese su nombre de usuario o escriba '0' para regresar al menú principal: \n")
    user = input().strip()
    if user == '0':
        os.system("cls")
        login()  
    ...
    UserTry = str(user)
    try:
        print("Ingrese la contraseña de su usuario o escriba '0' para regresar al menú principal:\n")
...
def admin():
    ...
    if password != "0000":
        print("Contraseña incorrecta. Regresando al menú principal.")
        os.system("pause")
        login()
        return
...
def cargaAutoresExcel():
    file_path = "C:\\Users\\Wilmer\\Desktop\\Python_practice\\LibrosDB.xlsx"
    try:
...
def BusquedaxNombre():
    ...
    try:
        ...
        cursor = conn.cursor()
        consulta = """
            SELECT ID_LIBRO, LIBRO, DISPONIBLE 
            FROM LIBROS 
            WHERE LIBRO LIKE ?
        """
        ...
        if resultados:
            ...
            for libro in resultados:
                disponibilidad = "Disponible" if libro[2] == 1 else "No disponible"
                print(f"ID: {libro[0]}, Título: {libro[1]}, Estado: {disponibilidad}")
            print("Ingrese el ID del libro que desea:")
            try:
                libro_id = int(input())
                libro_encontrado = next((libro for libro in resultados if libro[0] == libro_id), None)
                if libro_encontrado:
                    if libro_encontrado[2] == 0:  
                        print(f"El libro '{libro_encontrado[1]}' no está disponible.")
                        os.system("pause")
                        return Rentar()
...
def BusquedaxGenero():
    ...
    try:
        ...
        cursor.execute("SELECT ID_GENDER, GENDER FROM GENDERS")
        generos_disponibles = cursor.fetchall()

        if not generos_disponibles:
            print("No hay géneros disponibles en la base de datos.")
            return Rentar()

        # Mostrar los géneros disponibles con sus IDs
        print("Seleccione el ID del género que desea buscar:")
        for genero in generos_disponibles:
            print(f"ID: {genero[0]}, Género: {genero[1]}")

        # Pedir al usuario que ingrese el ID del género
        try:
            id_genero = int(input("Ingrese el ID del género: "))
            if not any(g[0] == id_genero for g in generos_disponibles):
                print("ID de género no válido. Intente nuevamente.")
                return BusquedaxGenero()
        except ValueError:
            print("Entrada no válida. Por favor, ingrese un número.")
            return BusquedaxGenero()

        # Buscar libros asociados al género seleccionado
...
def OpcUser():
    try:
        print("1)Si desea rentar\n2) si desea devolver un libro:\n")
        opc = int(input()) 
        if opc == 1:
            print("renta")
            Rentar()
...
#funcion del administrador para eliminar libros
def EliminarLibro():
    global libroCurrent
    print("Buscando el libro que desea eliminar...\n")
    BusquedaxNombre()  

    if libroCurrent == 0:
        print("No se seleccionó ningún libro. Operación cancelada.")
        return admin()
    ...
    try:
        ...
        confirmacion = input(f"¿Está seguro de que desea eliminar el libro con ID {libroCurrent}? (s/n): ").strip().lower()
        if confirmacion == 's':
            cursor.execute("DELETE FROM LIBROS WHERE ID_LIBRO = ?", (libroCurrent,))
            conn.commit()
            print(f"Libro con ID {libroCurrent} eliminado exitosamente.")
        else:
...
#Funcion del administrador para eliminar usuario
def EliminarUsuario():
    conn = None
    try:
        conn = sqlite3.connect("bibliotecaDB.db")
        ...
        try:
            ...
            usuario_encontrado = next((u for u in usuarios if u[0] == id_usuario), None)

            if not usuario_encontrado:
                print("ID de usuario no válido. Intente nuevamente.")
                return EliminarUsuario()

            confirmacion = input(f"¿Está seguro de que desea eliminar al usuario '{usuario_encontrado[1]}'? (s/n): ").strip().lower()
            if confirmacion == 's':
                cursor.execute("DELETE FROM USERS WHERE ID = ?", (id_usuario,))
                conn.commit()
                print(f"Usuario '{usuario_encontrado[1]}' eliminado exitosamente.")
                admin()
            else:
                print("Operación cancelada.")
        except ValueError:
...
def obtener_sinopsis_libro(nombre_libro, autor, api_key=None):
    ...
    try:
        ...
        # Primera búsqueda: nombre del libro + autor
        ...
        if "items" in data and len(data["items"]) > 0:
            ...
            if sinopsis:
                return {"sinopsis": sinopsis}
                            ...
                            # Segunda busqueda: solo el nombre del libro (Actualmente comentada por saturacion en base de datos, intentaré corregirla más adelante)
        # print(f"No se encontró sinopsis con el nombre y autor. Buscando solo con el nombre del libro: {nombre_libro}")
        # params = {"q": nombre_libro}
        # if api_key:
        #     params["key"] = api_key
