...
def UserCreator():
    global UserTry
    os.system("cls")
    print("Ingrese el nombre de usuario que desea guardar: ")
...
def Generos():
    ...
    GENDERS = ['Suspenso','Horror','Ciencia Ficcion','Educacion','Accion','Drama','Comedia','Romance','Fantasia','biografia','Otro']
...
def AddBooks(Book,disp,gen,aut,edit):
    ...
    count_id = cursor.fetchone()[0]
    carga = "INSERT INTO LIBROS (ID_LIBRO, LIBRO, DISPONIBLE, ID_GENERO, ID_AUTOR, ID_EDITORIAL) VALUES (?, ?, ?, ?, ?, ?)"
    cursor.execute(carga, (count_id, Book, disp, gen, aut, edit))       
    conn.commit()
    conn.close()

#Carga de libros por parte del excel
def cargaLibrosExcel():
    file_path = "LibrosDB.xlsx"  # Cambia esto por la ruta de tu archivo Excel
...
def BusquedaxNombre():
    ...
    try:
        conn = sqlite3.connect("bibliotecaDB.db")
        cursor = conn.cursor()
        consulta = "SELECT ID_LIBRO FROM LIBROS WHERE LIBRO LIKE ?"
        cursor.execute(consulta, (f"%{NombreAprox}%",)) 
        result = cursor.fetchone()
        libroCurrent = result[0] if result else None  
        
        if libroCurrent:
            print(f"Libro encontrado: ID {libroCurrent}")
        else:
...
def BusquedaxAutor():
    ...
    try:
        ...
        if IdAutTemp:
            ...
            consulta_libros = "SELECT LIBRO FROM LIBROS WHERE ID_AUTOR = ?"
...
def BusquedaxEditorial():
    ...
    try:
        ...
        if IdEditTemp:
            ...
            consulta_libros = "SELECT LIBRO FROM LIBROS WHERE ID_EDITORIAL = ?"
